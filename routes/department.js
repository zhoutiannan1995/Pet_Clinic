...
router.get('/allList', Auth, async function (req, res) {
  ...
  let sql = `SELECT * FROM pcdb.pc_department LIMIT ${curPage} , ${pageSize}`;
  if (req.query.pageSize == undefined || req.query.curPage == undefined) {sql = `SELECT * FROM pcdb.pc_department`}
  await connection.query('SELECT COUNT(*) FROM pcdb.pc_department', function (err, result) {
...
router.get('/find', Auth, function (req, res) {
  ...
  let sql = dpm_id ? `SELECT * FROM pcdb.pc_department WHERE dpm_id='${dpm_id}'` : `SELECT * FROM pcdb.pc_department WHERE dpm_name LIKE '%${dpm_name}%'`;
...
router.post('/addDepartment', Auth, function (req, res) {
  ...
  let sql = `INSERT INTO pcdb.pc_department (dpm_name, dpm_des) VALUES ('${dpm.dpm_name}','${dpm.dpm_des}')`;
  connection.query(sql, function (err, result) {
    if (err) res.send({ code: '999', msg: err });
    else {
...
router.post('/modifyDepartment', Auth, function (req, res) {
  let dpm = req.body.data,
    sql = `UPDATE pcdb.pc_department SET dpm_des='${dpm.dpm_des}' WHERE dpm_id='${dpm.dpm_id}'`;
  connection.query(sql, function (err, result) {
    if (err) res.send({ code: '999', msg: err });
    else {
...
router.post('/delDepartment', Auth, function (req, res) {
  let dpm = req.body.data,
    sql = `DELETE FROM pcdb.pc_department WHERE dpm_id=${dpm.dpm_id}`;
  connection.query(sql, function (err, result) {
    if (err) res.send({ code: '999', msg: err });
    else {
